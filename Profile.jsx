...
const Profile = () => {
    ...
    return (
        <>
            <MetaData title={`${user?.name} (@${user?.username}) • PingUp photos and videos`} />
                ...
                <div className="mt-16 xl:w-2/3 mx-auto">
                    ...
                    <div className="sm:flex w-full sm:py-8">
                        ...
                        <div className="flex flex-col gap-6 p-4 sm:w-2/3 sm:p-1">
                            <div className="flex items-center gap-8 sm:justify-start justify-between">
                                ...
                                {(loggedInUser.username === user.username) ? (
                                    <div className="flex gap-3 items-center">
                                        <Link to="/accounts/edit" className="border font-medium hover:bg-surface text-sm rounded px-2 py-1">Edit Profile</Link>
                                    ...
                                    <div className="flex gap-3 items-center">
                                        {follow ? (
                                            <>
                                                <button onClick={addToChat} className="border rounded px-2.5 py-[0.3rem] text-sm font-medium hover:bg-surface">Message</button>
                                            ...
                                            <button onClick={handleFollow} className="btn-primary text-sm text-white hover:shadow rounded px-6 py-1.5">Follow</button>
                                        )}
                            ...
                            <div className="max-w-full">
                                ...
                                {/* website removed from profile display */}
                            </div>
                        </div>
                    ...
                    <div className="border-t sm:ml-8 sm:mr-14">
                        ...
                        <div className="flex gap-12 justify-center">
                            <span onClick={() => setSavedTab(false)} className={`${savedTab ? 'text-muted-helper' : 'border-t border-black'} py-3 cursor-pointer flex items-center text-[13px] uppercase gap-3 tracking-[1px] font-medium`}>
                                {savedTab ? postsIconOutline : postsIconFill} posts</span>
                            {user._id === loggedInUser._id && (
                                <span onClick={() => setSavedTab(true)} className={`${savedTab ? 'border-t border-black' : 'text-muted-helper'} py-3 cursor-pointer flex items-center text-[13px] uppercase gap-3 tracking-[1px] font-medium`}>
                                    {savedTab ? savedIconFill : savedIconOutline} saved</span>
                            )}
                            <span className="py-3 flex items-center text-muted-helper text-[13px] uppercase gap-3 tracking-[1px] font-medium">
                                {reelsIcon} reels</span>
                            <span className="py-3 hidden sm:flex items-center text-muted-helper text-[13px] uppercase gap-3 tracking-[1px] font-medium">
