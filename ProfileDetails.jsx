...
const ProfileDetails = ({ setProfileToggle }) => {
    ...
    return (
        <ClickAwayListener onClickAway={() => setProfileToggle(false)}>
            <div className="absolute w-56 bg-surface rounded  drop-shadow top-14 right-0 md:right-72 md:top-14 border-theme">
                ...
                <div className="flex flex-col w-full overflow-hidden">
                    {tabs.map((el, i) => (
                        <Link to={el.redirect} className="flex items-center gap-3 p-2.5 text-sm pl-4 cursor-pointer hover:bg-surface" key={i}>
                            {el.icon}
                            {el.title}
                        </Link>
                    ))}
