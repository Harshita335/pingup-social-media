...
const Inbox = () => {
    ...
    useEffect(() => {
        ...
        socket.current.on('messageUpdated', (msg) => {
            dispatch({ type: 'MESSAGE_UPDATED', payload: msg });
        });
        socket.current.on('messageDeleted', (payload) => {
            dispatch({ type: 'MESSAGE_DELETED', payload });
        });
        socket.current.on('messageReaction', (payload) => {
            dispatch({ type: 'MESSAGE_REACTIONS', payload });
        });
                ...
                <div className="flex card h-full rounded w-full">
                    ...
                    {!userId ?
                        <div className="flex flex-col items-center justify-center w-full sm:w-4/6 gap-2">
                            <div className="w-24 h-24 flex items-center p-2 justify-center rounded-full border-theme border-2">
                            ...
                            <p className="text-muted-helper text-sm">Send private photos and messages to a friend or group.</p>
                            <button onClick={openModal} className="btn-primary rounded px-2.5 mt-2 py-1.5 text-white text-sm font-medium hover:drop-shadow-lg">Send Message</button>
                        </div>
                        :
                        <div className="flex flex-col justify-between w-full sm:w-4/6">

                            {/* header */}
                            <div className="flex py-3 px-6 border-b items-center justify-between border-theme">
                                <div className="flex gap-2 items-center">
                                    ...
                                    <span className="font-medium cursor-pointer text-theme">{friend.name}</span>
                                </div>
                                ...
                                {isTyping &&
                                    <>
                                        <div className="flex items-center gap-3 max-w-xs">
                                            ...
                                            <span className="text-sm text-muted-helper">typing...</span>
                                        </div>
                                        <div ref={scrollRef}></div>
                                    </>
                                }
